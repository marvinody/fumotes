#!/usr/bin/env bash
# call like ./sliding.sh nue.png endless_sliding_nue.gif
...
tmpfile=$(mktemp /tmp/sliding.XXXXXXX)
...
# hold each frame
tmpframeFolder=$(mktemp /tmp/sliding.XXXXXX -d)

# actually gen each frame
for ((d=0; d<$maxSize; d+=slidingAmt))
do
